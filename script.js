...
class ShaderBackground {
    ...
    constructor(canvasId) {
        ...
        // Disable WebGL on mobile devices only
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

        if (isMobile) {
            console.log('WebGL disabled on mobile device - using CSS fallback');
        ...
        try {
            this.init();
            this.isInitialized = true;  // Set BEFORE resize/start so they don't early-return
            this.resize();
            this.start();
        } catch (e) {
